...
<html>
  <head>
    ...
    <script type="text/javascript">
      ...
      $(function() {
        // When DOM is ready, select the container element with jQuery and call the jQCloud method, passing the word list as the first argument. Chainability of methods is maintained.
...
For each word object in your word array, you need to specify the following mandatory members:
...
You can also specify the following options for each word:
  ...
  * **callback** (function): a function to be called after this word is rendered. Within the function, `this` refers to the jQuery object for the `<span>` containing the word.
  * **handlers** (object): an object specifying event handlers that will bind to the word (e.g.: {click: function() { alert("it works!"); } })

### Cloud Options:

jQCloud accepts an object containing configuration options global to the cloud as the second argument:
...
Since drawing the cloud is rather computationally intensive, cloud rendering isn't instantaneous. If you want to make sure that some code executes after the cloud is rendered, put it in the callback:
...
1.0.0 API redesign (warning: backward compatibility is broken)
